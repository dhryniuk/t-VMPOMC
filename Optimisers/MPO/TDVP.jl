export TDVP, NonMarkovianOptimize!
...
function NonMarkovianSweepLindblad!(sample::Projector, ρ_sample::T, optimizer::TDVPl1{T}, list_A, δ::Float64, i::Int64) where {T<:Complex{<:AbstractFloat}} 
...
function NonMarkovianUpdate!(optimizer::TDVPl1{T}, sample::Projector, list_A, δ::Float64, i::Int64) where {T<:Complex{<:AbstractFloat}} #... the ensemble averages etc.
    ...
    ρ_sample::T = MPO(params,sample,A)
    cache.L_set = L_MPO_strings!(cache.L_set, sample,A,params,cache)
    cache.Δ = ∂MPO(sample, cache.L_set, cache.R_set, params, cache)./ρ_sample

    #Sweep lattice:
    local_L = NonMarkovianSweepLindblad!(sample, ρ_sample, optimizer, list_A, δ, i)
...
function NonMarkovianReconfigure!(optimizer::TDVPl1{T}, δ::Float64, i::Int64, list_grad) where {T<:Complex{<:AbstractFloat}} #... the gradient tensor
    ...
    #Reconfigure gradient:
    grad::Array{T,3} = (data.L∂L-data.ΔLL)/N_MC
    display(grad)
    push!(list_grad,grad)
    #display(list_grad)
    grad = zeros(T,optimizer.params.χ,optimizer.params.χ,4)
    kernel_riemann_sum=0
    for k in 1:i
        kernel_riemann_sum += δ*K(-δ*(i-k))
    end
    println(kernel_riemann_sum)
    for k in 1:i
        grad+= δ*K(-δ*(i-k))*list_grad[k+1]/kernel_riemann_sum
    end
    display(grad)
    #sleep(5)
    #display(inv(data.S))

    flat_grad::Vector{T} = reshape(grad,4*params.χ^2)
    flat_grad = inv(data.S)*flat_grad
    data.∇ = reshape(flat_grad,params.χ,params.χ,4)
end
...
export NonMarkovianComputeGradient!

function NonMarkovianComputeGradient!(optimizer::TDVPl1{T}, list_A, δ::Float64, i::Int64) where {T<:Complex{<:AbstractFloat}}
    ...
    for _ in 1:optimizer.sampler.N_MC
        ...
        #Compute local estimators:
        NonMarkovianUpdate!(optimizer, sample, list_A, δ, i) 
...
function NonMarkovianOptimize!(optimizer::TDVPl1{T}, δ::Float64, i::Int64, list_A) where {T<:Complex{<:AbstractFloat}}
    ...
    #Convolution:
    #∇ = zeros(T,optimizer.params.χ,optimizer.params.χ,4)
    #for k in 1:i
    #    ∇+= δ*K(dt*(i-k))*list_∇[k]
    #end
    #optimizer.optimizer_cache.∇ = ∇
    ...
    #sleep(5)
    #error()
    #NonMarkovianReconfigure!(optimizer, δ, i, list_grad)
    Reconfigure!(optimizer)
    ∇ = optimizer.optimizer_cache.∇
    #display(optimizer.optimizer_cache.∇)
    #println()
